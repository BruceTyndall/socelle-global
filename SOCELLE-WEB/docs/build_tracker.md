...
| V2-HUBS-06 | CRM Hub — full non-shell | CRM | CRM Agent | Phase 5 | ✅ COMPLETE (2026-03-08) — CRM Dashboard (TanStack Query v5, overdue task alerts, quick-link cards), Contacts (full CRUD + search/filter + CSV export + Intelligence tab with contact insights), Companies (full CRUD + CSV export), Tasks (create/complete/delete + priority + overdue detection + filter tabs + CSV export), Segments (filter rule builder + live preview count + delete), EditContact + EditCompany with delete confirmation, sidebar nav with CRM sub-pages. All routes auth-gated (ProtectedRoute + ModuleRoute MODULE_CRM). LIVE/DEMO labeling via isLive pattern. tsc 0 errors, build pass. |
