...
| SITE-POWER-01 | Dual pricing routes, orphan /home route. | OPEN | — |
| MOBILE-POWER-01 | Flutter screens are UI shells without MODULE_* entitlement gates. | OPEN | — |

## 🚀 NEW WOs (REALITY CHECK AUDIT REMEDIATION)
...
| INTEL-MONETIZATION-01 | Intelligence | Owner request: turn taxonomy + signal + event data into paid brand/provider intelligence products. Current slice: monetization playbook + `user_tag_preferences` foundation, onboarding seed persistence, anonymous session preference memory, authenticated signal save/like persistence, profile-ready signal libraries, additive Intelligence reranking grounded in live aggregates rather than hardcoded stats, and live DB apply for `user_tag_preferences` + `user_signal_engagements` + merge RPCs. | READY FOR REVIEW | P1 | WO-TAXONOMY-03 |
| INTEL-MONETIZATION-02 | Intelligence | Owner request: apply first-class Intelligence Channels backed by live `intelligence_channels` + `intelligence_channel_tags`, weighted taxonomy performance views, and personalized channel rails on public + portal Intelligence surfaces. | READY FOR REVIEW | P1 | WO-TAXONOMY-03 |
| SITE-ONBOARD-01 | Public Site | In-app onboarding flow (Identity Scan → Shadow Audit → Signal Match → Gate) | OPEN | P1 | B |
| MOBILE-PUSH-01 | Mobile | Push notification handlers for high-priority signals + rebooking warnings | OPEN | P2 | B |
| SALES-AUTOFILL-01 | Sales | Proposal builder auto-fill from triggering signal context | OPEN | P2 | A |
...
| WO-TAXONOMY-01 | Owner request: add flat `public.taxonomy_tags` import table for the 500+ cross-hub taxonomy. Scope: schema + indexes + `moddatetime('updated_at')` trigger + RLS + source-aligned query utilities; import source = `socelle_taxonomy_full_500plus.csv` via Supabase Table Editor. | Team 1 | READY FOR REVIEW | none | `docs/qa/verify_WO-TAXONOMY-01.json` | Live DB applied 2026-03-12: migration `20260313000042` recorded remotely and 584 taxonomy rows imported. | db+code+docs |
| WO-TAXONOMY-02 | Owner request: add `rss_tag_rules` + `rss_item_tags` + `auto_tag_rss_items` using `taxonomy_tags.tag_code` as the canonical RSS tagging source; wire `ingest-rss` to auto-tag newly upserted items; capture operating specs in `INTEL_TAXONOMY_AND_SIGNALS.md` + `AUTO_TAGGING_RULES.md`. | Team 1 | READY FOR REVIEW | WO-TAXONOMY-01 | `docs/qa/verify_WO-TAXONOMY-02.json` | Live DB applied 2026-03-12: migration `20260313000043` recorded remotely, `ingest-rss` forced to v22, 66 active rules confirmed, and `rss_item_tags` now holds 603 live rows across 19 tag codes. | db+edge+docs |
| WO-TAXONOMY-03 | Owner request: add canonical taxonomy fields to `market_signals`, create tag-performance/tag-health SQL views over `market_signals` + `rss_item_tags` + `platform_events`, and persist signal interaction analytics with tag payloads so we can rank, compare, and clean pro beauty tags over time. | Team 1 | READY FOR REVIEW | WO-TAXONOMY-02 | `docs/qa/verify_WO-TAXONOMY-03.json` | Live DB applied 2026-03-12: migration `20260313000044` recorded remotely, `rss-to-signals` redeployed to v23, 52 older RSS signals backfilled with `source_feed_id`, and a 200-row promotion passed after duplicate-fingerprint handling was fixed. | db+edge+analytics+docs |
| INTEL-MONETIZATION-02 | First-class Intelligence Channels: `intelligence_channels` + weighted tag mappings + live channel performance views + personalized channel rails across public and portal Intelligence surfaces. | Team 1 | READY FOR REVIEW | WO-TAXONOMY-03, INTEL-MONETIZATION-01 | `docs/qa/verify_INTEL-MONETIZATION-02.json` | Live DB applied 2026-03-12: migration `20260313000047` recorded remotely, 14 active channels + 91 tag mappings are live, commits `539129f` + `0470a3a` are on `origin/main`, Cloudflare now serves `index-jVbdOPNg.js` / `index-BC175c89.css`, and direct public `/intelligence` now resolves to the intended `/waitlist` prelaunch surface instead of the 404 shell. Public anonymous traffic remains intentionally prelaunch-gated. | db+code+docs |
