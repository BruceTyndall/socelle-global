// ── FeaturedCardGridModule — V2-INTEL-01 ─────────────────────────────
// Wrapper: renders featured cards from top signals with images.
// Data source: useModuleAdapters().featuredCards (from useIntelligence)
...
interface FeaturedCardGridModuleProps {
  eyebrow?: string;
  headline?: string;
  dark?: boolean;
  columns?: 2 | 3;
...
export function FeaturedCardGridModule({
  eyebrow,
  headline,
  dark = false,
  columns = 3,
}: FeaturedCardGridModuleProps) {
  const { featuredCards, loading, isLive } = useModuleAdapters();

  if (loading) return <ModuleLoading label="Loading featured signals..." dark={dark} />;
  if (featuredCards.length === 0) return <ModuleEmpty label="No featured signals available." dark={dark} />;
