...
export default function CreditDashboard() {
  const { balance, transactions, isLive, loading, error, reload, tierAllocations } = useCreditBalance();

  const overageStatus = useMemo(() => {
    if (balance.remaining < 0) return 'overage';
    if (balance.remaining < balance.allocated * 0.1) return 'low';
    return 'healthy';
  }, [balance]);

  const exportCsv = useCallback(() => {
    const headers = ['Action', 'Credits Deducted', 'Signal ID', 'Timestamp'];
    const rows = transactions.map((t) => [
      t.action,
      String(t.credits_deducted),
      t.signal_id ?? '',
      t.created_at,
    ]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Used This Month', value: formatNumber(balance.used_this_month), icon: TrendingUp, color: 'text-accent' },
          { label: 'Allocated (Tier)', value: `${formatNumber(balance.allocated)} (${balance.tier})`, icon: Gauge, color: 'text-graphite' },
          {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {Object.entries(tierAllocations).map(([tier, credits]) => (
            ...
            >
              ...
              <p className="text-xs text-graphite/40 font-sans">credits/mo</p>
              {tier === balance.tier && (
            ...
            <table className="w-full text-sm font-sans">
              <thead>
                <tr className="border-b border-graphite/8">
                  <th className="px-6 py-3 text-left text-xs font-semibold text-graphite/50 uppercase tracking-wider">Action</th>
                  <th className="px-6 py-3 text-right text-xs font-semibold text-graphite/50 uppercase tracking-wider">Credits</th>
                  <th className="px-6 py-3 text-left text-xs font-semibold text-graphite/50 uppercase tracking-wider">Signal Ref</th>
              ...
              <tbody className="divide-y divide-graphite/5">
                {transactions.map((t) => (
                  <tr key={t.id} className="hover:bg-accent-soft/30 transition-colors">
                    <td className="px-6 py-3 font-medium text-graphite">{t.action}</td>
                    <td className="px-6 py-3 text-right text-graphite font-semibold">-{t.credits_deducted}</td>
