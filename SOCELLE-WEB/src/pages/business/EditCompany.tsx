  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        <Link to={`/portal/crm/companies/${id}`} className="w-8 h-8 rounded-full border border-pro-stone/30 flex items-center justify-center hover:border-accent/30 transition-colors">
          <ArrowLeft className="w-4 h-4 text-pro-warm-gray" />
        </Link>
        <h1 className="text-xl font-semibold text-pro-charcoal flex-1">Edit Company</h1>
        <button
      ...
      {showDeleteConfirm && (
        <div className="bg-signal-down/5 border border-signal-down/20 rounded-xl p-5">
          <p className="text-sm font-medium text-pro-charcoal mb-2">Delete this company?</p>
          <p className="text-xs text-pro-warm-gray mb-4">This will permanently remove {company.name}. Contacts linked to this company will not be deleted but will lose their company association.</p>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-pro-stone/30 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={form.industry} onChange={set('industry')} className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal focus:outline-none focus:border-accent/50" />
          </div>
          <div>
            <label className="block text-xs font-medium text-pro-warm-gray mb-1.5">Website</label>
            <input type="url" value={form.website} onChange={set('website')} className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal focus:outline-none focus:border-accent/50" />
          </div>
        </div>
        ...
        <div className="border-t border-pro-stone/20 pt-4">
          ...
          <div className="space-y-3">
            <input type="text" value={form.address_line1} onChange={set('address_line1')} placeholder="Address Line 1" className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal placeholder:text-pro-warm-gray focus:outline-none focus:border-accent/50" />
            <input type="text" value={form.address_line2} onChange={set('address_line2')} placeholder="Address Line 2" className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal placeholder:text-pro-warm-gray focus:outline-none focus:border-accent/50" />
            <div className="grid grid-cols-4 gap-3">
              <div className="col-span-2">
                <input type="text" value={form.city} onChange={set('city')} placeholder="City" className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal placeholder:text-pro-warm-gray focus:outline-none focus:border-accent/50" />
              </div>
              <input type="text" value={form.state} onChange={set('state')} placeholder="State" className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal placeholder:text-pro-warm-gray focus:outline-none focus:border-accent/50" />
              <input type="text" value={form.zip} onChange={set('zip')} placeholder="ZIP" className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal placeholder:text-pro-warm-gray focus:outline-none focus:border-accent/50" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" value={form.annual_revenue} onChange={set('annual_revenue')} className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal focus:outline-none focus:border-accent/50" />
          </div>
          <div>
            <label className="block text-xs font-medium text-pro-warm-gray mb-1.5">Employee Count</label>
            <input type="number" value={form.employee_count} onChange={set('employee_count')} className="w-full h-10 px-3 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal focus:outline-none focus:border-accent/50" />
          </div>
        </div>

        <div>
          <label className="block text-xs font-medium text-pro-warm-gray mb-1.5">Notes</label>
          <textarea value={form.notes} onChange={set('notes')} rows={3} className="w-full px-3 py-2 border border-pro-stone/30 rounded-lg text-sm text-pro-charcoal focus:outline-none focus:border-accent/50 resize-none" />
        </div>

        <div className="flex justify-end gap-3 pt-2">
          <Link to={`/portal/crm/companies/${id}`} className="h-10 px-5 text-sm text-pro-warm-gray hover:text-pro-charcoal inline-flex items-center">Cancel</Link>
