      ...
      <div className="space-y-8">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="font-serif text-3xl text-pro-charcoal">Audience Segments</h1>
                {!isLive && (
        ...
        {showForm && (
          <div className="bg-white rounded-xl border border-pro-stone p-6 shadow-sm space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="font-serif text-lg text-pro-charcoal">Create Segment</h2>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Filter className="w-4 h-4 text-pro-navy" />
                <span className="text-sm font-sans font-semibold text-pro-charcoal">Filter Rules</span>
              </div>
              ...
              <div className="space-y-3">
                {rules.map((rule, idx) => (
                  <div key={rule.id} className="flex items-center gap-3 flex-wrap">
                    {idx > 0 && (
                      <span className="text-xs font-sans font-medium text-pro-warm-gray w-8 text-center">AND</span>
                    )}
                    <select
                      value={rule.field}
                      onChange={(e) => updateRule(rule.id, 'field', e.target.value)}
                      className="h-9 px-3 rounded-lg border border-pro-stone text-sm font-sans text-pro-charcoal bg-white focus:outline-none focus:ring-2 focus:ring-pro-navy/20"
                    >
                      {FIELD_OPTIONS.map((f) => (
                    ...
                    <input
                      ...
                      placeholder="Value"
                      className="flex-1 min-w-[120px] h-9 px-3 rounded-lg border border-pro-stone text-sm font-sans text-pro-charcoal placeholder:text-pro-warm-gray/50 focus:outline-none focus:ring-2 focus:ring-pro-navy/20"
                    />
                    {rules.length > 1 && (
                      <button
              ...
              <button
                onClick={addRule}
                className="mt-3 inline-flex items-center gap-1.5 text-xs font-sans font-medium text-pro-navy hover:text-pro-navy/80 transition-colors"
              >
        ...
        <div className="bg-white rounded-xl border border-pro-stone shadow-sm overflow-hidden">
          <div className="p-6 border-b border-pro-stone">
            <h2 className="font-serif text-xl text-pro-charcoal">All Segments</h2>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 px-4">
              <Users className="w-10 h-10 text-pro-warm-gray/40 mx-auto mb-3" />
              <p className="text-sm text-pro-warm-gray font-sans">No segments created yet</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm font-sans">
                ...
                <tbody className="divide-y divide-pro-stone/50">
                  {segments.map((seg) => (
                    <tr key={seg.id} className="hover:bg-pro-cream/30 transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-medium text-pro-charcoal">{seg.name}</p>
                        {seg.description && (
                          <p className="text-xs text-pro-warm-gray mt-0.5">{seg.description}</p>
                        )}
                      </td>
                      <td className="px-6 py-4 text-right">
                        ...
                        <span className="text-pro-warm-gray ml-1">contacts</span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {(seg.filters ?? []).slice(0, 3).map((f, i) => (
                            <span
                              key={i}
                              className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-pro-cream text-pro-charcoal"
                            >
                              {f.field} {f.operator} {String(f.value)}
                            </span>
                          ))}
                          {(seg.filters ?? []).length > 3 && (
                            <span className="text-xs text-pro-warm-gray">
