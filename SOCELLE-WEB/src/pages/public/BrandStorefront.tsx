...
/* STATUS: DEMO — data sourced from hardcoded brandIntelligence.ts maps, not DB.
   W10-07+ will wire to real purchase-correlation data. */

function ProfessionalsAlsoBoughtSection({ brandSlug }: { brandSlug: string }) {
  const alsoBought = getProfessionalsAlsoBought(brandSlug);
...
/* STATUS: DEMO — professionalCount, trending, adoptionCount all from
   hardcoded brandIntelligence.ts maps. W10-07+ will wire to real data. */

function PeerAdoptionBanner({ brandSlug }: { brandSlug: string }) {
  const peerData = getBrandPeerData(brandSlug);
  const trending = isBrandTrending(brandSlug);
  const adoptionCount = getBrandAdoptionCount(brandSlug);
        ...
        <div className="bg-white rounded-card shadow-panel overflow-hidden">
          ...
          {slug && (
            <>
              <SectionDivider />
              <div className="py-6">
                <PeerAdoptionBanner brandSlug={slug} />
              </div>
            </>
          )}
          ...
          {slug && (
            <>
              <SectionDivider />
              <ProfessionalsAlsoBoughtSection brandSlug={slug} />
            </>
          )}
