// ── EducationArticles — V2-HUBS-14 ───────────────────────────────────
// Public education articles listing: /education/articles
// Data: cms_posts where space='education', status='published'
// Pearl Mineral V2 tokens only.

import { useState, useMemo } from 'react';
import { Link } from 'react-router-dom';
import { Helmet } from 'react-helmet-async';
import { Clock, ArrowRight, Search, GraduationCap, Award } from 'lucide-react';
import MainNav from '../../components/MainNav';
import SiteFooter from '../../components/sections/SiteFooter';
import { useEducationContent } from '../../lib/cms';
...
/** Safely extract a string field from metadata Json */
...
/** Safely extract a boolean field from metadata Json */
function getMetaBool(metadata: Json | null, key: string): boolean {
...
function ArticleCard({ post }: { post: CmsPost }) {
  const readTime = estimateReadingTime(post);
  const difficulty = getMetaString(post.metadata, 'difficulty');
  const hasCeCredit = getMetaBool(post.metadata, 'ce_credit');

  return (
    <Link
      to={`/education/articles/${post.slug}`}
    ...
    >
      ...
      <div className="p-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          {hasCeCredit && (
            <span className="flex items-center gap-1 text-[10px] font-semibold text-[#5F8A72] bg-[#5F8A72]/10 px-2 py-0.5 rounded-full">
...
export default function EducationArticles() {
  ...
  const { articles, isLoading, isLive, error } = useEducationContent({
    status: 'published',
  });
  ...
  const schemaData = {
    ...
    '@type': 'CollectionPage',
    name: 'Education Articles | SOCELLE',
    description:
      'Professional education articles, treatment guides, and continuing education content for beauty and medspa professionals.',
    url: 'https://socelle.com/education/articles',
    publisher: {
      '@type': 'Organization',
      name: 'SOCELLE',
    ...
    mainEntity: {
      ...
      itemListElement: paginated.slice(0, 10).map((p, i) => ({
        '@type': 'ListItem',
        position: i + 1,
        url: `https://socelle.com/education/articles/${p.slug}`,
        name: p.title,
      })),
    },
  };
  ...
  return (
    <>
      <Helmet>
        <title>Education Articles | SOCELLE</title>
        <meta
          name="description"
          content="Professional education articles, treatment guides, and continuing education content for beauty and medspa professionals."
        />
        <meta property="og:title" content="Education Articles | SOCELLE" />
        <meta
          property="og:description"
          content="Professional education articles, treatment guides, and continuing education content for beauty and medspa professionals."
        />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://socelle.com/education/articles" />
        <meta property="og:image" content="https://socelle.com/og-image.svg" />
        <meta name="robots" content="index, follow" />
        <link
          rel="canonical"
          href="https://socelle.com/education/articles"
        />
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="text-center mb-12">
            ...
            <p className="mt-3 text-[#141418]/60 font-sans text-lg max-w-xl mx-auto">
              Treatment guides, technique breakdowns, and continuing education
              for beauty and medspa professionals.
            </p>
            {!isLive && !isLoading && (
          ...
          {!isLoading && !error && filtered.length === 0 && (
            <div className="text-center py-20">
              <GraduationCap className="w-8 h-8 text-[#141418]/20 mx-auto mb-4" />
              <p className="text-[#141418]/40 font-sans text-lg">
                {searchTerm || activeCategory
                  ? 'No articles match your filters.'
                  : 'No education articles published yet.'}
              </p>
            </div>
          )}
