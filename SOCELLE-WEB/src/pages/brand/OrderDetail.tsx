  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto space-y-6 animate-pulse">
        <div className="h-5 bg-accent-soft/30 rounded w-24" />
        <div className="h-8 bg-accent-soft/30 rounded w-56" />
        <div className="bg-white rounded-xl border border-accent-soft p-6 space-y-4">
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto space-y-4">
        <Link
          to="/brand/orders"
          className="inline-flex items-center gap-1.5 text-sm font-sans text-graphite/60 hover:text-graphite transition-colors"
        >
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-sans text-2xl text-graphite">{order.order_number}</h1>
      ...
      <div className="bg-white rounded-xl border border-accent-soft p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-xs font-semibold font-sans text-graphite/60 uppercase tracking-wider">Message reseller</p>
        </div>
          ...
          <>
            ...
            <div className="px-5 py-4 border-t border-accent-soft bg-background/50">
              <div className="flex justify-between items-center">
                <span className="text-sm font-semibold font-sans text-graphite">Order total</span>
                <span className="text-base font-semibold font-sans text-graphite">
      ...
      {(order.tracking_number || order.tracking_carrier) && (
        <div className="bg-white rounded-xl border border-accent-soft p-5">
          <div className="flex items-center gap-2 mb-3">
            <Truck className="w-4 h-4 text-graphite/60" />
            <p className="text-xs font-semibold font-sans text-graphite/60 uppercase tracking-wider">
              Tracking
            </p>
          </div>
          <div className="space-y-1">
            {order.tracking_carrier && (
              <p className="text-sm text-graphite font-sans">
                <span className="text-graphite/60">Carrier:</span> {order.tracking_carrier}
              </p>
            )}
            {order.tracking_number && (
              <p className="text-sm text-graphite font-sans">
                <span className="text-graphite/60">Tracking #:</span>{' '}
      ...
      {order.return_status === 'requested' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
          ...
          {order.return_reason && (
            <p className="text-sm text-graphite font-sans mb-4">{order.return_reason}</p>
          )}
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={resolveReturnLoading}
              className="px-4 py-2 rounded-lg text-sm font-sans font-medium border border-accent-soft text-graphite hover:bg-accent-soft disabled:opacity-50"
            >
              Reject
            </button>
          </div>
      ...
      {nextOptions.length > 0 && (
        <div className="bg-white rounded-xl border border-accent-soft overflow-hidden">
          <div className="px-6 pt-5 pb-4 border-b border-accent-soft">
            <p className="text-xs font-semibold font-sans text-graphite/60 uppercase tracking-wider">
              Update order status
            </p>
          </div>
          <div className="px-6 py-5 space-y-4">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium font-sans transition-colors border ${
                    newStatus === status
                      ...
                      : 'bg-white text-graphite border-accent-soft hover:border-graphite'
                  }`}
                >
            ...
            {showTracking && (
              <div className="rounded-lg border border-accent-soft/60 bg-background p-4 space-y-3">
                <p className="text-xs font-semibold font-sans text-graphite">
                  Tracking details <span className="text-red-400">*</span>
                </p>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="block text-xs text-graphite/60 font-sans">
                      Tracking number <span className="text-red-400">*</span>
                    </label>
                    <div className="flex items-center">
                      <Truck className="absolute ml-3 w-4 h-4 text-graphite/60 pointer-events-none" />
                      <input
                        type="text"
                        value={trackingNumber}
                        onChange={e => setTrackingNumber(e.target.value)}
                        placeholder="1Z999AA10123456784"
                        className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-accent-soft text-sm font-mono text-graphite placeholder:text-accent-soft focus:outline-none focus:ring-2 focus:ring-graphite/20"
                      />
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <label className="block text-xs text-graphite/60 font-sans">Carrier</label>
                    <select
                      value={trackingCarrier}
                      onChange={e => setTrackingCarrier(e.target.value)}
                      className="w-full px-4 py-2.5 rounded-lg border border-accent-soft text-sm font-sans text-graphite bg-white appearance-none focus:outline-none focus:ring-2 focus:ring-graphite/20"
                    >
            ...
            {newStatus && (
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="text-sm font-sans text-graphite/60 hover:text-graphite transition-colors"
                >
                  Cancel
                </button>
                {saved && (
      ...
      {order.notes && (
        <div className="bg-white rounded-xl border border-accent-soft p-5">
          <p className="text-xs font-semibold font-sans text-graphite/60 uppercase tracking-wider mb-2">
            Notes from retailer
          </p>
      ...
      {order.brand_notes && (
        <div className="bg-white rounded-xl border border-accent-soft p-5">
          ...
          <p className="text-sm text-graphite font-sans leading-relaxed">{order.brand_notes}</p>
        </div>
      )}
