...
const STATUS_COLORS: Record<string, string> = {
  ...
  duplicate: 'bg-pro-stone text-pro-charcoal',
};
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-serif text-pro-navy">
              CRM Hub<span className="text-pro-gold">.</span>
            </h1>
            {!isLive && (
            ...
            <table className="w-full text-sm font-sans">
              <thead>
                <tr className="border-b border-pro-stone bg-pro-cream/50">
                  ...
                  <th className="text-left px-4 py-3 font-medium text-pro-charcoal">Business</th>
                  ...
                  <tr key={row.id} className="hover:bg-pro-cream/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Mail className="w-3.5 h-3.5 text-pro-warm-gray flex-shrink-0" />
                        <div>
                          {row.contact_name && (
                            <p className="text-pro-navy font-medium">{row.contact_name}</p>
                          )}
                          <p className="text-pro-warm-gray text-xs">{row.email}</p>
                        </div>
                      </div>
                    </td>
    ...
    <div className="bg-white border border-pro-stone rounded-xl p-5 space-y-3">
      {[...Array(5)].map((_, i) => (
        <div key={i} className="flex items-center gap-4 animate-pulse">
          <div className="w-8 h-8 rounded-lg bg-pro-stone flex-shrink-0" />
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="bg-white border border-pro-stone rounded-xl p-12 text-center">
      <Users className="w-12 h-12 text-pro-stone mx-auto mb-4" />
...
function ComingSoonCard({ title, description }: { title: string; description: string }) {
  return (
    <div className="bg-white border border-pro-stone rounded-xl p-12 text-center">
      <Building2 className="w-12 h-12 text-pro-stone mx-auto mb-4" />
