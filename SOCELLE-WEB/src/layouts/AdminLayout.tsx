...
export default function AdminLayout() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <Link to="/admin/dashboard" className={`flex items-center gap-3 px-4 py-5 border-b border-accent-soft ${collapsed ? 'justify-center' : ''}`}>
        ...
        {!collapsed && (
          <div>
            ...
            <p className="font-sans text-xs text-graphite/60 mt-0.5">Admin Portal</p>
          </div>
        )}
      </Link>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2">
        {NAV_GROUPS.map((group) => (
          <div key={group.label} className="mb-5">
            {!collapsed && (
              <p className="px-3 mb-1.5 text-[10px] font-bold uppercase tracking-widest text-graphite/60/60 font-sans">
                {group.label}
              </p>
            )}
      ...
      {/* Footer */}
      <div className={`border-t border-accent-soft p-3 space-y-1 ${collapsed ? 'flex flex-col items-center' : ''}`}>
        <Link
          to="/"
          className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-graphite/60 hover:text-graphite hover:bg-accent-soft transition-colors font-sans ${collapsed ? 'justify-center' : ''}`}
        ...
        <button
          ...
          className={`flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm text-graphite/60 hover:text-red-600 hover:bg-red-50 transition-colors font-sans ${collapsed ? 'justify-center' : ''}`}
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Desktop Sidebar */}
      <aside
        className={`hidden lg:flex flex-col bg-white border-r border-accent-soft transition-all duration-200 flex-shrink-0 ${
          collapsed ? 'w-16' : 'w-56'
        }`}
      >
        <SidebarContent />
        ...
        <button
          ...
          className="absolute bottom-20 -right-3 w-6 h-6 bg-white border border-accent-soft rounded-full flex items-center justify-center shadow-sm hover:shadow-md transition-shadow text-graphite/60 hover:text-graphite hidden lg:flex"
        ...
        <header className="bg-white border-b border-accent-soft h-14 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden lg:block">
              <p className="text-sm font-semibold text-graphite font-sans">Admin Portal</p>
            </div>
          </div>
          {user && (
