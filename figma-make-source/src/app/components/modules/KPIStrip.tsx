...
import { type KPI, formatTimeAgo, getFreshnessColor } from '../../data/mock';
...
export function KPIStrip({ kpis, title, dark = false }: KPIStripProps) {
  ...
  const glowColor = dark ? 'rgba(63,84,101,0.03)' : 'rgba(63,84,101,0.05)';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {title && (
          <div className="text-center mb-14">
            <span className="eyebrow text-[#141418]/50 mb-4 block">Live Data</span>
          ...
          {kpis.map((kpi) => (
            <div key={kpi.id} className="text-center group panel-mineral p-6 relative">
              ...
              <div className="flex items-center justify-center gap-2 mb-2">
                <span className={`flex items-center gap-0.5 text-xs ${kpi.delta >= 0 ? 'text-[#5F8A72]' : 'text-[#8E6464]'}`} style={{ fontFamily: 'var(--font-mono)' }}>
                  {kpi.delta >= 0 ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                  {kpi.delta >= 0 ? '+' : ''}{kpi.delta}%
                </span>
              </div>
              <div className="flex items-center justify-center gap-2">
                <span className={`${badgeText} text-[10px] px-2 py-0.5 rounded-full border ${badgeBorder}`} style={{ fontFamily: 'var(--font-mono)' }}>
                  {kpi.confidence}% confidence
                </span>
              </div>
              <div className={`text-[10px] mt-1.5 ${getFreshnessColor(kpi.updatedAt)}`} style={{ fontFamily: 'var(--font-mono)' }}>
