...
import { type Signal, formatTimeAgo, getFreshnessColor, getFreshnessDot } from '../../data/mock';
...
export function SignalTable({ signals, title, showViewAll, dark = false }: SignalTableProps) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {title && (
          <div className="flex items-end justify-between mb-10">
            <div>
              <span className="eyebrow text-[#141418]/50 mb-3 block">Signal Feed</span>
          ...
          <table className="w-full">
            <thead>
              <tr className={`border-b ${headerBorder}`}>
                ...
                <th className="text-right pb-4 pr-4"><SortHeader label="Δ 30d" sortKeyName="trendValue" /></th>
            ...
            <tbody>
              {sorted.map((signal) => (
                <tr
                  key={signal.id}
                  className={`border-b ${rowBorder} ${rowHover} transition-colors group cursor-pointer`}
                >
                  ...
                  <td className="py-4 pr-4 text-right">
                    <span className={`flex items-center justify-end gap-1 text-sm ${signal.trend === 'up' ? 'text-[#5F8A72]' : signal.trend === 'down' ? 'text-[#8E6464]' : 'text-gray-400'}`} style={{ fontFamily: 'var(--font-mono)' }}>
                      {signal.trend === 'up' ? <TrendingUp className="w-3.5 h-3.5" /> : signal.trend === 'down' ? <TrendingDown className="w-3.5 h-3.5" /> : <Minus className="w-3.5 h-3.5" />}
                  ...
                  <td className={`py-4 text-right hidden sm:table-cell text-xs ${getFreshnessColor(signal.updatedAt)}`} style={{ fontFamily: 'var(--font-mono)' }}>
