import { formatTimeAgo, getFreshnessColor } from '../../data/mock';
...
export function EvidenceStrip({ cells, dark = false }: EvidenceStripProps) {
        ...
        <div className="flex flex-wrap justify-center gap-6 lg:gap-0 lg:divide-x lg:divide-inherit">
          {cells.map((cell) => (
            <div key={cell.id} className={`flex-1 min-w-[140px] text-center px-6 lg:px-10 ${borderClass}`}>
              ...
              {cell.updatedAt && (
                <div className={`text-[10px] mt-1 ${getFreshnessColor(cell.updatedAt)}`} style={{ fontFamily: 'var(--font-mono)' }}>
