      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <Rocket className="w-8 h-8 text-pro-charcoal" />
            <div>
              <h2 className="text-2xl font-semibold text-pro-charcoal">Implementation Planner</h2>
              <p className="text-sm text-pro-warm-gray">Rollout plans, readiness scoring, and onboarding tools</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <select
              ...
              className="px-4 py-2 border border-pro-stone rounded-lg text-sm"
            >
              {menus.map(menu => (
            ...
            <button
              ...
              className="px-4 py-2 bg-pro-navy text-white rounded-lg hover:bg-pro-charcoal disabled:opacity-50 flex items-center gap-2"
            >
        ...
        <div className="bg-pro-cream border border-pro-stone rounded-lg p-4 flex gap-3">
          <FileText className="w-5 h-5 text-pro-charcoal flex-shrink-0 mt-0.5" />
        ...
        <div className="p-6">
          {activeTab === 'readiness' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-pro-charcoal">Service Readiness Profiles</h3>
              ...
              {readinessData.length === 0 ? (
                ...
                <div className="space-y-3">
                  {readinessData.map((item: any) => (
                    <div key={item.id} className="border border-pro-stone rounded-lg p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <div className="font-medium text-pro-charcoal">
                      ...
                      <div className="grid grid-cols-4 gap-4 text-sm">
                        <div>
                          <div className="text-pro-warm-gray mb-1">Training</div>
                          <div className="font-medium">{item.training_complexity}</div>
                          {item.estimated_training_hours && (
                            <div className="text-xs text-pro-warm-gray">{item.estimated_training_hours}h</div>
                          )}
                        </div>
                        <div>
                          <div className="text-pro-warm-gray mb-1">Products</div>
                          <div className="font-medium">{item.product_count_required}</div>
                        </div>
                        <div>
                          <div className="text-pro-warm-gray mb-1">Contraindications</div>
                          <div className="font-medium">{item.contraindication_sensitivity}</div>
                        </div>
                        <div>
                          <div className="text-pro-warm-gray mb-1">Staff Level</div>
                      ...
                      {item.prerequisites && (
                        <div className="mt-3 pt-3 border-t border-pro-stone">
                          <div className="text-xs text-pro-warm-gray mb-1">Prerequisites:</div>
                          <div className="text-sm text-pro-charcoal">{item.prerequisites}</div>
                        </div>
                      )}
          ...
          {activeTab === 'rollout' && rolloutPlan && (
            <div className="space-y-6">
              <div className="bg-pro-ivory border border-pro-stone rounded-lg p-4">
                <h3 className="font-semibold text-pro-charcoal mb-3">{rolloutPlan.plan.plan_name}</h3>
                <div className="grid grid-cols-4 gap-4 text-sm">
                  <div>
                    <div className="text-pro-warm-gray">Total Services</div>
                    <div className="text-lg font-semibold text-pro-charcoal">{rolloutPlan.plan.total_services}</div>
                  </div>
                  <div>
                    <div className="text-pro-warm-gray">Avg Risk Score</div>
                    <div className="text-lg font-semibold text-pro-charcoal">{Math.round(rolloutPlan.plan.avg_risk_score || 0)}/100</div>
                  </div>
                  <div>
                    <div className="text-pro-warm-gray">Training Hours</div>
                    <div className="text-lg font-semibold text-pro-charcoal">{(rolloutPlan.plan.total_training_hours || 0).toFixed(1)}h</div>
                  </div>
                  <div>
                    <div className="text-pro-warm-gray">Phases</div>
                    <div className="text-lg font-semibold text-pro-charcoal">{rolloutPlan.plan.total_phases}</div>
                  </div>
                </div>
              </div>
              ...
              {brandNarrative && (
                <div className="bg-pro-cream border border-pro-stone rounded-lg p-4">
                  <div className="text-sm text-pro-navy">{brandNarrative.introduction}</div>
                  {brandNarrative.plan_specific_highlights && brandNarrative.plan_specific_highlights.length > 0 && (
                    <ul className="mt-3 space-y-1 text-sm text-pro-navy">
              ...
              {rolloutPlan.phases.map((phase: any) => (
                <div key={phase.phase_number} className="border border-pro-stone rounded-lg overflow-hidden">
                  <div className="bg-pro-stone px-4 py-3 border-b border-pro-stone">
                    <div className="flex items-center justify-between">
                      <h4 className="font-semibold text-pro-charcoal">{phase.phase_name}</h4>
                    ...
                    {phase.items.map((item: any) => (
                      <div key={item.id} className="p-4">
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="font-medium text-pro-charcoal">{item.service_name}</div>
                            <div className="text-sm text-pro-warm-gray">{item.protocol_name}</div>
                            <div className="text-xs text-pro-warm-gray mt-1">{item.phase_rationale}</div>
                          </div>
                          <div className="text-right">
                            ...
                            {item.training_hours && (
                              <div className="text-xs text-pro-warm-gray">{item.training_hours}h training</div>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="border border-pro-stone rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <Package className="w-5 h-5 text-pro-charcoal" />
                    <h3 className="font-semibold text-pro-charcoal">Backbar Products</h3>
                  </div>
                  <div className="space-y-2">
                    {(openingOrder.backbar_products || []).slice(0, 10).map((product: any, idx: number) => (
                      <div key={idx} className="text-sm">
                        <div className="font-medium text-pro-charcoal">{product.product_name}</div>
                        <div className="text-xs text-pro-warm-gray">{product.protocol_name}</div>
                      </div>
                    ))}
                    {openingOrder.backbar_products.length > 10 && (
                      <div className="text-sm text-pro-warm-gray italic">
                ...
                <div className="border border-pro-stone rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingUp className="w-5 h-5 text-pro-charcoal" />
                    <h3 className="font-semibold text-pro-charcoal">Retail Assortment</h3>
                  </div>
                  <div className="space-y-2">
                    {(openingOrder.retail_products || []).map((product: any, idx: number) => (
                      <div key={idx} className="text-sm">
                        <div className="font-medium text-pro-charcoal">{product.product_name}</div>
                        {product.msrp && (
                          <div className="text-xs text-pro-warm-gray">MSRP: ${product.msrp}</div>
                        )}
                      </div>
                    ))}
                  </div>
                  {openingOrder.estimated_retail_investment && (
                    <div className="mt-4 pt-4 border-t border-pro-stone">
                      <div className="text-sm text-pro-warm-gray">Estimated Investment</div>
                      <div className="text-lg font-semibold text-pro-charcoal">
              ...
              {openingOrder.recommended_launch_window && (
                <div className="bg-pro-cream border border-pro-stone rounded-lg p-4">
                  ...
                  {openingOrder.seasonal_rationale && (
                    <div className="text-xs text-pro-navy mt-1">{openingOrder.seasonal_rationale}</div>
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-4">
                {openingOrder.setup_checklist && (
                  <div className="border border-pro-stone rounded-lg p-4">
                    <div className="font-semibold text-pro-charcoal mb-3">Setup Checklist</div>
                    <div className="space-y-2 text-sm">
                      {openingOrder.setup_checklist.map((section: any, idx: number) => (
                        <div key={idx}>
                          <div className="font-medium text-pro-charcoal">{section.category}</div>
                          <div className="text-xs text-pro-warm-gray">{section.items.length} items</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {openingOrder.training_checklist && (
                  <div className="border border-pro-stone rounded-lg p-4">
                    <div className="font-semibold text-pro-charcoal mb-3">Training Checklist</div>
                    <div className="space-y-2 text-sm">
                ...
                {openingOrder.launch_checklist && (
                  <div className="border border-pro-stone rounded-lg p-4">
                    <div className="font-semibold text-pro-charcoal mb-3">Launch Checklist</div>
                    <div className="space-y-2 text-sm">
          ...
          {activeTab === 'brand' && brandNarrative && (
            <div className="space-y-6">
              <div className="bg-pro-ivory border border-pro-stone rounded-lg p-6">
                <h3 className="text-xl font-semibold text-pro-charcoal mb-4">{brandNarrative.headline}</h3>
              ...
              {brandNarrative.sections && brandNarrative.sections.map((section: any, idx: number) => (
                <div key={idx} className="border border-pro-stone rounded-lg p-4">
                  <h4 className="font-semibold text-pro-charcoal mb-2">{section.title}</h4>
                  <p className="text-sm text-pro-charcoal mb-3">{section.content}</p>
