...
1. **`/.Codex/AGENTS.md`** — this file (governance, stop conditions, launch gates)
...
**After reading those, check which WO you are executing and read the relevant skill(s) from `/.Codex/skills/` before starting work.**
...
You have **99 operational skills** at `/.Codex/skills/`. These are auditors, validators, and generators that enforce quality gates.
...
2. After completing any WO or sub-task, the agent MUST run the corresponding verification skill(s) from `/.Codex/skills/`.
...
These pages are transactional flows (auth forms, commerce checkout) that legitimately have no `live_data`, empty state, or CRUD. They MUST NOT be modified per AGENTS.md §B (NEVER MODIFY).
