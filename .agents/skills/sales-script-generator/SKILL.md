...
description: "Creates personalized sales scripts, demo talking points, and discovery call frameworks from prospect profiles and SOCELLE intelligence data. Use this skill whenever someone asks to write a sales script, prepare for a demo, build a discovery call guide, create pitch talking points, or personalize outreach for a specific prospect. Also triggers on: 'sales script', 'demo prep', 'discovery call', 'pitch deck talking points', 'prospect briefing', 'demo walkthrough', 'sales call prep', 'personalized pitch'. IMPORTANT: This skill generates scripts for inbound/warm prospects only — AGENTS.md §G prohibits cold outreach. All scripts lead with intelligence value, never product features."
---

# Sales Script Generator

Creates personalized, intelligence-first sales scripts for warm prospects and inbound leads. SOCELLE's acquisition model is on-site (no cold outreach per AGENTS.md §G), so these scripts are for prospects who have already expressed interest through `/request-access` or inbound channels. Every script positions intelligence as the value hook — the product demo is a data walkthrough, not a feature tour.
