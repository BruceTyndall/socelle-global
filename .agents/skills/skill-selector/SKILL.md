---
name: skill-selector
description: ALWAYS use this skill when you need to find, match, recommend, or auto-invoke the right skill for a user prompt. Triggers on "which skill", "find skill", "match skill", "suggest skill", "skill for this", "what skill should I use", "route to skill", "skill lookup", "auto-select skill". Scans the full .Codex/skills/ library, matches prompt keywords and intent to skill descriptions and trigger phrases, and recommends or invokes the best-fit skill. Essential meta-skill for navigating large skill libraries (90+).
---
...
### Hooks Integration
Add to `.Codex/hooks.json` (when supported):
```json
{
  "pre-prompt": {
    "skill-selector": {
...
- **Retire** — If Codex's native skill routing becomes sufficient, this becomes redundant — retire gracefully
